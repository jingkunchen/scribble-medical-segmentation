...
parser.add_argument('--root_path', type=str,
                    default='../data/MS-CMRSeg_lge_c0', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='MS-CMRSeg_lge_c0/WeaklySeg_pCE_sematic_matching_weight_1_1_final', help='experiment_name')
...
def get_fold_ids(fold):
    all_cases_set = ["patient{:d}_".format(i) for i in range(1,46)]
    fold1_testing_set = [
        "patient{:d}_".format(i) for i in range(1, 10)]
    fold1_training_set = [
        i for i in all_cases_set if i not in fold1_testing_set]

    fold2_testing_set = [
        "patient{:d}_".format(i) for i in range(10, 19)]
    fold2_training_set = [
        i for i in all_cases_set if i not in fold2_testing_set]

    fold3_testing_set = [
        "patient{:d}_".format(i) for i in range(19, 28)]
    fold3_training_set = [
        i for i in all_cases_set if i not in fold3_testing_set]

    fold4_testing_set = [
        "patient{:d}_".format(i) for i in range(28, 37)]
    fold4_training_set = [
        i for i in all_cases_set if i not in fold4_testing_set]

    fold5_testing_set = [
        "patient{:d}_".format(i) for i in range(37, 46)]
    fold5_training_set = [
...
def test_single_volume(case, net, test_save_path, FLAGS):
    h5f = h5py.File(FLAGS.root_path +
                    "/MS-CMRSeg_lge_c0_training_volumes/{}".format(case), 'r')
    image = h5f['image'][:]
    label = h5f['label'][:]
    ...
    org_img_path = "../data/MS-CMRSeg_lge_c0_training/{}_norm.nii.gz".format(case)
...
def Inference(FLAGS):
    ...
    all_volumes = os.listdir(
        FLAGS.root_path + "/MS-CMRSeg_lge_c0_training_volumes")
    image_list = []
    for ids in test_ids:
